<!--主题框架开始-->
<?php $this->_cdata['_title']="轮播图－".$this->_cdata['_title'];?>
        ...
        <div class="row">
            <div class="col-xs-12">
                ...
                <div class="row">
                    <div class="col-xs-12">
                        <div class="table-header">
                            共<span style="color:yellow;" class="allcount"><?=$pic['page']['count']?></span>条数据
                        </div>
                        <div class="table-responsive">
                            <div id="sample-table-2_wrapper" class="dataTables_wrapper" role="grid">
                                ...
                                <div class="row" style="padding:5px;">
                                        ...
                                        <div id="sample-table-2_length" class="dataTables_length">
                                            <label>位置：
                                                <select class="sooneSearch sooneSearch_calsses" name="sample-table-2_length" aria-controls="sample-table-2">
                                                        <option value="index" <?php if($classes=="index"){ echo "selected='selected'";}?> >首页</option>
                                                </select>
                                            </label>
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="dataTables_filter" id="sample-table-2_filter">
                                            <label>
                                                <a href="/admin/other/rollpic_add"><button class="btn btn-sm btn-warning">新增</button></a>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <table id="sample-table-2" class="table table-striped table-bordered table-hover">
                                    ...
                                    <tbody>
                                    <?php foreach ($pic['data'] as $k=>$v):?>
                                        <tr class="delMark<?=$v['id'];?>">
                                            <td class="center"><?=$v['id']?></td>
                                            <td title="<?=$v['title']?>"><?=mb_substr($v['title'],0,9,"utf-8");?></td>
                                            <td><img width="60px;" height="40px;" src="<?=$v['img']?>"/></td>
                                            <td><?=mb_substr($v['link'],0,12,"utf-8");?></td>
                                            <td><?=$v['order']?></td>
                                            <td><?=$v['createtime']?></td>
                                            <td>
                                                <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                    <a class="blue" target="_blank" href="/">
                                                        <i class="icon-zoom-in bigger-130"></i>
                                                    </a>
                                                    <a class="green" href="/admin/other/rollpic_edit?id=<?=$v['id']?>">
                ...
                <script type="text/javascript">
                    ...
                    jQuery('#paginate').pagination(<?=$pic['page']['count']?>,{
                        current_page:<?=$pic['page']['curpage']?>,
                        items_per_page:<?=$pic['page']['per']?>,
                        link_to:"/admin/other/rollpic/__id__?classes=<?=$classes?>"
                    });
                    $(document).ready(function() {
                        $(".sooneSearch").change(function () {
                            var classes = $(".sooneSearch_calsses").val();
                            window.location.href="/admin/other/rollpic?classes="+classes;
                        });
                    });
                </script>
                <!--page-->
